 ...
 */
contract AaveV3ScWbtcAdapter is IAdapter {
    using SafeTransferLib for ERC20;
    using SafeTransferLib for WETH;

    ERC20 public constant wbtc = ERC20(C.WBTC);
    ...
    // Aave v3 "aEthwbtc" token (supply token)
    ERC20 public constant aWbtc = ERC20(C.AAVE_V3_AWBTC_TOKEN);
    ...
    function withdraw(uint256 _amount) external override {
        pool.withdraw(address(wbtc), _amount, address(this));
    }
    ...
    function getCollateral(address _account) external view override returns (uint256) {
        return aWbtc.balanceOf(_account);
    }
    ...
    function getMaxLtv() external view override returns (uint256) {
        (, uint256 ltv,,,,,,,,) = aaveV3PoolDataProvider.getReserveConfigurationData(address(wbtc));
