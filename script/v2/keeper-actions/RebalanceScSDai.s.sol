...
contract RebalanceScSDai is scCrossAssetYieldVaultRebalanceScript {
    ...
    // minProfitToReinvest - the minimum amount of WETH profit (converted to SDAI) that needs to be made for reinvesting to make sense (ie gas costs < profit made)
    // maxProfitSellSlippage (scCrossAssetYieldVaultRebalanceScript) - the maximum amount of slippage allowed when selling WETH profit for SDAI
    // spark investable amount percent - the percentage of the available funds that can be invested for a specific adapter (all have to sum up to 100% or 1e18)
    // spark target ltv - the target loan to value ratio for a specific adapter. Set to 0 for unused or unsupported adapters!
