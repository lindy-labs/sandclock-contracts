...
import {NoProfitsToSell, FlashLoanAmountZero, EndUsdcBalanceTooLow, FloatBalanceTooLow} from "src/errors/scErrors.sol";
...
import {BaseV2Vault} from "certora/harness/BaseV2Vault.sol";
...
contract scUSDCv2 is BaseV2Vault {
    ...
    constructor(address _admin, address _keeper, ERC20 _asset, ERC4626 _scWETH, PriceConverter _priceConverter, Swapper _swapper)
    ...
    function _repayAllDebtAndWithdrawCollateral() internal {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (debt > 0) _repay(0, debt);
            if (collateral > 0) _withdraw(0, collateral);
        }
    }
